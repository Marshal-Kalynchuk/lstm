...
N_PAST = 360 # 0.5 days
N_FUTURE = 360 # 15minutes
N_TEST = 5000
BATCH_SIZE = 512
DATASET_SIZE = 700000
EPOCHS = 4
STEPS_PER_EPOCH = 100 # (DATASET_SIZE - N_TEST)//BATCH_SIZE
N_PARAMS = 5
...
def create_rolling_window(data_tf, n_past, n_future):
    ...
    for i in range(n_past, len(data_tf) - n_future + 1):
        seq = data_tf[i - n_past:i]
        temp = data_tf[i + n_future - 1: i + n_future]['open']
        target = 0
        if (temp.iloc[0] > seq.iloc[-1]['open']):
            target = 1
        elif (temp.iloc[0] < seq.iloc[-1]['open']):
            target = 0
...
def data_generator(data_df, n_past, n_future, batch_size):
    while True:
        ...
        for i in range(batch_size):
            ...
            temp = data_df[idx + n_future - 1:idx + n_future]['open']
            target = 0
            if (temp.iloc[0] > seq.iloc[-1]['open']):
                target = 1
            elif (temp.iloc[0] < seq.iloc[-1]['open']):
...
def create_lstm():
    ...
    regressor.add(CuDNNLSTM(units = 64, return_sequences=True, input_shape=(N_PAST, N_PARAMS)))
    regressor.add(Dropout(0.2))

    regressor.add(CuDNNLSTM(units = 48, return_sequences=True))
    regressor.add(Dropout(0.2))

    regressor.add(CuDNNLSTM(units = 32, return_sequences=True))
    regressor.add(Dropout(0.2))

    regressor.add(CuDNNLSTM(units = 16))
    regressor.add(Dropout(0.2))
...
pred_y = regressor.predict(test_x).flatten()
